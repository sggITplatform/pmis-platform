...
public class FileUploadUtils
{
	...
	public FileUploadUtils(String destinationPath)
	{
	...
	public boolean Upload()
	{
		...
		try
		{
			...
			//如果没有上传目的目录，则创建它
			FileUtils.createFileIfNoExists(dstPath);
			//如果没有临时目录，则创建它
			if (StringUtils.isNotEmpty(this.tempPath))
			{
				FileUtils.createFileIfNoExists(tempPath);
			}
			...
			while (iter.hasNext())
			{
				...
				if (!item.isFormField())
				{
					...
					if (result)
					{
						...
						String ext = "." + FileUtils.getTypePart(name);
						try
						{
							//保存上传的文件到指定的目录
							...
							else
							{
								...
								FileUtils.createFileIfNoExists(uploadfilename);
